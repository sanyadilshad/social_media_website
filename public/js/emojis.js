...
function handleEmojiClick(emoji) {
  document.getElementById("id-chat-message").value =
    document.getElementById("id-chat-message").value + emoji;
  document.getElementById("emoji-menu").classList.toggle("hidden");
}

function renderEmojisMenu() {
  let emojiBtns = EMOJIS_LIST.map((item) => {
    return `<button class='border border-gray-200 rounded p-2' onclick='handleEmojiClick("${item.emoji}")'>${item.emoji}</button>`;
  }).join("");
  document.getElementById("emoji-menu").innerHTML =
    '<div><div class="flex justify-between bg-gray-200"><div class="text-xs p-1">Smileys, Emotion, Flags</div><button onclick="showEmojis()" class="me-2"><i class="fa-solid fa-times"></i></button></div><div class="flex flex-wrap bg-white p-2 h-[200px] overflow-y-auto overflow-x-hidden gap-1">' +
    emojiBtns +
    "</div></div>";
}

function showEmojis() {
  let el = document.getElementById("emoji-menu");
  let eBtn = document.getElementById("emoji-btn");
  el.classList.toggle("hidden");
  if (!el.classList.contains("hidden")) {
    el.style.left = eBtn.offsetLeft + "px";
    el.style.top = eBtn.offsetTop - 220 + "px";
  }
}
